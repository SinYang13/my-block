  ...
  try {
    ...
    for (const doc of querySnapshot.docs) {
      ...
      if (data.image) {
        ...
        data.image = `https://firebasestorage.googleapis.com/v0/b/myblock-wad.appspot.com/o/events%2F${encodeURIComponent(
          data.image
        )}?alt=media`;
      }
      ...
      try {
        ...
        console.error(
          `Error reading registrations for event ${doc.id}:`,
          error
        );
      }
      ...
      try {
        ...
        if (data.price && data.price.includes("$")) {
          ...
          if (!isNaN(price)) {
            ...
            let totalAttendees = data.registrations.reduce(
              (sum, registration) => {
                return (
                  sum +
            ...
            // Calculate the revenue based on the total number of attendees
            data.revenue = price * totalAttendees;
          } else {
...
async function deleteEvent(eventId) {
  ...
  try {
    ...
    const deleteAttendeesPromises = attendeesSnapshot.docs.map((attendeeDoc) =>
      deleteDoc(attendeeDoc.ref)
    );
